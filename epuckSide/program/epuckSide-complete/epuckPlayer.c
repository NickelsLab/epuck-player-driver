...
char *G_camera_buffer=buffer;
unsigned int G_image_bytes_size = 0; // start with empty cam buffer
...
void read_floor_sensors() {
   ...
   for (j=0;j<6;j+=2) {
       ve_send_int(buffer[j]&0xff + buffer[j+1]<<8);
   }
   send_char(1); // indicate end of 
}
...
void config_camera()
{
  ...
  if (G_image_bytes_size>BUFFER_SIZE) {
      send_char(-1); /* error - not enough space reserved */
  } else
  {
  send_char(1);/* It signalize the end of operation. */
  }
